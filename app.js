...
`;
  ...
  const App = {
    ...
    getEnabledMayoMadnessModules() {
      ...
      return MAYO_MADNESS_SUBMODULE_KEYS.filter((key) => {
        const counts = this.getModuleCounts(key);
        return this.state.settings.modulesEnabled[key] && counts && counts.available > 0;
      });
    },
    ...
    async submitFeedback(event) {
      ...
      try {
        ...
        formData.append('_subject', `Meridian feedback · ${this.currentLevel || 'AP Human Geography'}`);
